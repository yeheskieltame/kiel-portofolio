...
export const AdminEducation = () => {
  ...
  const handleEditEducation = (edu: any) => {
    // If image_url doesn't exist in the education object, add it with an empty string
  ...
  const handleAddEducation = () => {
    ...
    // Add temporary IDs for the courses since the Education interface expects them
    const educationToAdd = {
      ...newEducation,
      courses: newEducation.courses.map((course, index) => ({
        id: index, // Temporary ID, will be replaced by database
        name: course.name,
        link: course.link
      }))
    };

    addEducation(educationToAdd);
  ...
  const handleAddCourse = (isNew: boolean) => {
    if (isNew) {
      ...
      setCurrentEducation(prev => ({
        ...prev!,
        courses: [...prev!.courses, { id: prev!.courses.length, name: '', link: '' }]
      }));
    }
  };
          ...
          {currentEducation && (
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <Input 
                  id="edit-image" 
                  value={currentEducation.image_url || ''} 
                  onChange={(e) => setCurrentEducation({...currentEducation, image_url: e.target.value})}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              ...
              <Input 
                id="add-image" 
                value={newEducation.image_url} 
                onChange={(e) => setNewEducation({...newEducation, image_url: e.target.value})}
                placeholder="URL to provider image (optional)"
              />
            </div>
            <div className="space-y-2">
